...
(function($) {
    ...
    if (H$("?id")) {
        ...
        formobj._url = "<%=_module%>/<%=_entity%>/search.act?<%=Cnst.RN_KEY%>=0&<%=Cnst.AB_KEY%>=.enus,.info,.form,_fork,_text";
    } else {
        formobj.formBox.attr("action", "<%=_module%>/<%=_entity%>/create.act");
        formobj._url = "<%=_module%>/<%=_entity%>/select.act?<%=Cnst.RN_KEY%>=0&<%=Cnst.AB_KEY%>=.enus,.info,.form,_fork,_text";
    }
