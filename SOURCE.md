...
一个动作脚本即前端定制谈到的表单 ID 目录下的 `__main__.jsp` 文件，如果此文件存在，则对应资源的所有更新改查等操作均会转到此处，但仍然可以通过 ActionRunner 调用原始的动作程序，这时 `__main__.jsp` 充当一个过滤器的角色，可以通过 ActionHelper 对输入输出数据进行改写。
